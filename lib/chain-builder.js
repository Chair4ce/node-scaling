...
const TASK_PATTERNS = [
  ...
  },
  {
    ...
    stages: ['extract', 'enrich', 'analyze', 'synthesize'],
...
const STAGE_TEMPLATES = {
  extract: (task, data) => ({
    ...
    prompt: `Extract all relevant data points, facts, and signals from the input. One item per line. Include any quantitative signals (upvotes, counts, prices) as severity/importance indicators. Focus on: ${task}`,
  }),
  ...
  filter: (task) => ({
    ...
    mode: 'single',
    perspective: 'filter',
    prompt: `Review the extracted items. Remove noise, duplicates, and low-relevance items. Score remaining items 1-10 for relevance to: ${task}. Output only items scoring 6+. Format: [score] item`,
  ...
  analyze: (task) => ({
    ...
    mode: 'single',
    perspective: 'analyst',
    prompt: `Analyze these data points for patterns, trends, and implications. What story does the data tell? What's surprising? What's missing? Focus on: ${task}`,
    inputTransform: 'merge',
  }),
  ...
  enrich: (task) => ({
    ...
    webSearch: true,
    prompt: `Take these data points and add context. Cross-reference with current market data, competitor landscape, and industry trends. Add depth without adding noise. Focus on: ${task}`,
    inputTransform: 'merge',
  }),
  ...
  challenge: (task) => ({
    ...
    prompt: `Play devil's advocate on this analysis. What are the blind spots? What assumptions are unsupported? What risks are being ignored? What would make this analysis wrong? Be specific and constructive. Context: ${task}`,
    inputTransform: 'merge',
  }),
  ...
  optimize: (task) => ({
    ...
    prompt: `Refine this output. Fix inconsistencies, sharpen conclusions, add missing nuance. Make it actionable. Context: ${task}`,
    inputTransform: 'merge',
  }),
  ...
  synthesize: (task) => ({
    ...
    prompt: `Synthesize all previous analyses into a single coherent output. Resolve contradictions. Highlight key insights and actionable takeaways. Be concise and opinionated. Context: ${task}`,
    inputTransform: 'merge',
  }),
...
function detectTaskType(task) {
  ...
  return {
    name: 'general',
    stages: ['extract', 'analyze', 'synthesize'],
