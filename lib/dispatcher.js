...
class Dispatcher {
  ...
  async orchestrate(phases, options = {}) {
    ...
    // Announce to user
    if (!options.silent) {
      this.announce(`INITIALIZING SWARM... (${phases.length} phases, up to ${this.maxNodes} workers)`);
    }
    ...
    // Announce completion
    if (!options.silent) {
      this.announce(`SWARM COMPLETE ✓ ${successCount}/${totalCount} tasks in ${(totalDuration/1000).toFixed(1)}s`);
    }
